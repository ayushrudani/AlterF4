const { ApiSucceessResponse, ApiErrorResponse } = require("../utils/ApiResponse.js");
...
const getAllCorp = async (req, res) => {
    try{
        ...
        const crops = await Crop.find().skip(pageNumber * pageSize).limit(pageSize);
        ...
        return res.json(new ApiSucceessResponse(200 , data , "All crops fetched successfully"  ))
    }catch(error){
...
const getCorpById = async (req, res) => {
    try{
        ...
        return res.json(new ApiSucceessResponse(200 , crop , "Crop fetched successfully"  ))
    }catch(error){
        return res.json(new  ApiErrorResponse(500 , error.message));
    }
  };
